...
def main():
    ...
    for fn in sorted(os.listdir(FRONT)):
        ...
        sbytes = conv(spath) if os.path.exists(spath) else nbytes
        off[intl] = slot * SIZE * SIZE * 2
        normal += nbytes
        shiny += (sbytes if len(sbytes) == len(blank) else blank)
        slot += 1
    ...
    with open(OUT_S, "w") as s:
        ...
        # absolute paths: the assembler runs from build/, and this .s is git-ignored / regenerated locally
    ...
    with open(OUT_C, "w") as c:
        ...
        c.write("  const uint8_t* b = shiny ? mon_front_shiny_blob : mon_front_blob;\n")
        c.write("  return (const uint16_t*)(b + o);\n")
        c.write("}\n")

    kb = len(normal) / 1024.0
    print("front sprites: %d species, %.1f KiB x2 (normal+shiny) = %.2f MiB ROM"
          % (slot, kb, 2 * kb / 1024.0))
